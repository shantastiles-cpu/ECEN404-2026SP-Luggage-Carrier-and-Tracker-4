...
# ── Height compensation ───────────────────────────────────────────────────────
# If the anchors and tag are at different heights, the UWB measures the 3D
# slant distance. Set the height difference in metres to project back to the
# horizontal plane: horizontal = sqrt(range² - height_diff²)
#
# ANCHOR_HEIGHT_M : height of the anchors above the floor (metres)
# TAG_HEIGHT_M    : height of the tag above the floor (metres)
# If both are at the same height, set both to 0.0 (no correction applied)
ANCHOR_HEIGHT_M = 0.0   # metres
TAG_HEIGHT_M    = 0.0   # metres
...
class AnchorReader:
    ...
    def _parse(self, raw: bytes):
        try:
            ...
            # ── Height compensation ───────────────────────────────────────────
            # Project 3D slant range to horizontal distance
            height_diff = abs(ANCHOR_HEIGHT_M - TAG_HEIGHT_M)
            if height_diff > 0.0:
                horiz_sq = rng**2 - height_diff**2
                if horiz_sq <= 0.0:
                    log.debug(f"[Anchor {self.anchor_num}] Range {rng:.3f} m less than height diff {height_diff:.3f} m — discarded")
                    return
